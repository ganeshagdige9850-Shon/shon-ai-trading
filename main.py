    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if ticker == 'CL=F':  # Crude Oil
                # WTI Crude: USD/barrel × INR rate = MCX price per barrel
                price = round(price_usd * USD_INR)
            elif ticker == 'GC=F':  # Gold
                # Gold: USD/oz × INR × conversion for 10g
                price = round(price_usd * USD_INR / 31.1 * 10)
            elif ticker == 'SI=F':  # Silver
                # Silver: USD/oz × INR × conversion for 1kg
...
def calculate_greeks(spot, strike, premium, days_to_expiry, iv, opt_type='CE'):
    """Calculate Option Greeks - Fixed Delta signs"""
    # Calculate moneyness (distance from ATM)
    distance_pct = abs(strike - spot) / spot
    
    # Delta calculation - FIXED to ensure correct signs
    if opt_type == 'CE':  # CALL option
        if strike < spot:  # ITM - In The Money
            delta = 0.70 - distance_pct
        elif abs(strike - spot) / spot < 0.02:  # ATM - At The Money
            delta = 0.50
        else:  # OTM - Out of The Money
            delta = 0.35 - distance_pct
        # Ensure CALL delta is always positive (0 to 1)
        ...
        if strike > spot:  # ITM - In The Money
            ...
            delta = -0.35 + distance_pct
        # Ensure PUT delta is always negative (-1 to 0)
