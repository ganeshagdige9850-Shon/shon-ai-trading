...
def get_yahoo_price(ticker):
    """Get MCX prices with CORRECT conversions"""
    global USD_INR
    try:
        r = requests.get(
        ...
        if r.status_code == 200:
            ...
            if ticker == 'CL=F':  # Crude Oil
                ...
                # USD/oz to Rs/10g: ($/oz × INR) / 31.1035 × 10
                ...
                # USD/oz to Rs/kg: $/oz × INR × 32.1507
...
def analyze_comprehensive(price, change, cfg, budget=15000):
    ...
    if ind['trend'] == 'BULLISH':
        direction, strike = 'CALL', atm + gap
    elif ind['trend'] == 'BEARISH':
        direction, strike = 'PUT', atm - gap
    else:
        direction, strike = 'WAIT', atm
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg += '8 Assets: NSE (5) + MCX (3)\n'
    msg += 'All conversions CORRECT! ✅\n\n'
...
async def markets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg += '\n✅ Gold ~Rs72k, Silver ~Rs84k, Crude ~Rs6k'
    await update.message.reply_text(msg)
...
async def analyze(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if asset not in ASSETS:
        await update.message.reply_text('❌ Invalid asset!')
        return
    
    cfg = ASSETS[asset]
    price, change = get_yahoo_price(cfg['ticker']) if cfg['type'] == 'mcx' else angel.get_price(cfg['token'])
...
async def recommend(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key, cfg in ASSETS.items():
        ...
        if price:
            ...
            all_data.append({'asset': cfg['name'], 'price': price, 'change': change, 'a': a, 'lot': cfg['lot']})
    
    if not all_data:
        await update.message.reply_text('❌ No data!')
        return
    
    best = max(all_data, key=lambda x: x['a']['confidence'] if x['a']['fits_budget'] else 0)
    a = best['a']
    ...
    msg += f'GREEKS:\nΔ={g["delta"]:+.3f}, Γ={g["gamma"]}, Θ={g["theta"]}\n\n'
    msg += f'RISK:\nEntry: Rs{a["premium"]}\n'
